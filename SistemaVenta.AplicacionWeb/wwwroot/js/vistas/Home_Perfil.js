...
$("#btnGuardarCambios").click(function () {
    if ($("#txtCorreo").val().trim() == "") {
        toastr.warning("", "El campo está vacio, debes completar el correo.")
        $("#txtCorreo").focus()
        return;
    }

    if ($("#txtTelefono").val().trim() == "") {
        toastr.warning("", "El campo está vacio, debes completar el teléfono.")
        $("#txtTelefono").focus()
        return;
    }
        ...
        function (respuesta) {
            if (respuesta) {
                ...
                fetch("/Home/GuardarPerfil", {
                    ...
                    .then(responseJson => {
                        if (responseJson.estado) {
                            swal("Listo!", "Los cambios fueron realizado, satisfactoriamente.", "success")
                        }
                        else {
                            swal("Lo sentimos", responseJson.mensaje, "error")
                        }
...
$("#btnCambiarClave").click(function () {
    ...
    if ($("#txtClaveNueva").val().trim() != $("#txtConfirmarClave").val().trim()) {
        toastr.warning("", "Las contraseñas no son iguales, favor de inteneter nuevamente. .")
        return;
    }
    ...
    fetch("/Home/CambiarClave", {
        ...
        .then(responseJson => {
            if (responseJson.estado) {
                swal("Listo!", "Su contraseña fué actualizada, satisfactoriamente.", "success")
                $("input.input-validar").val("");
            }
            else {
                //swal("Lo sentimos", responseJson.mensaje, "error")
                swal("Lo sentimos", "La contraeña actual ingresada no es correcta.", "error")
            }
        })
})